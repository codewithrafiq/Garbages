...
from mtcnn.MTCNN import DetectionMtcnn
...
class SAVE_EMBDED:
    def __init__(self):
        ...
        self.model = ArcFaceModel(size=self.input_size,
                         ...
                         training=False)
        self.ckpt_path = tf.train.latest_checkpoint('/home/sanaullah/Downloads/Face-Recognition-IBUS-main/checkpoints/' + self.sub_name)
    ...
    def save_multiple_embed(self,path):
        ...
        for image_name in glob.glob(path+"*"):
            ...
            if not image_name:
                ...
                cv2.imwrite("./"+str(name)+".png",cv2.cvtColor(face, cv2.COLOR_BGR2RGB) )
                # print(face.shape)
