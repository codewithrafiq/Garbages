...
from project.ml.libs.recognition.modules import utils
from project.ml.libs.recognition.modules.utils import l2_norm
from scipy.spatial.distance import euclidean
from project.ml.libs.recognition.modules.models import ArcFaceModel
from project.ml.libs.recognition.modules.evaluations import get_val_data, perform_val
from project.ml.libs.recognition.modules.utils  import set_memory_growth, load_yaml, l2_norm
...
class SAVE_EMBDED:
    def __init__(self):
        ...
        self.model = ArcFaceModel(size=self.input_size,
                         ...
                         training=False)
        self.ckpt_path = tf.train.latest_checkpoint('project/ml/checkpoints/' + self.sub_name)
    ...
    def save_multiple_embed(self,path):
        ...
        with open("project/ml/information/embdding.pkl", "wb") as fi:
    ...
    def register(self,image, name):
        ...
        nam = np.array(names)
        embds_dict = dict(zip(nam, embd))
        with open("project/ml/information/embdding.pkl", "rb") as fi:
            file = pickle.load(fi)
        file.update(embds_dict)
        out_put = open("project/ml/information/embdding.pkl", "wb")
