...
from project.ml.src.main import ALL_FUNCTIONS
now = datetime.now()
...
@router.post("/recognition_frame/")
async def recognition_frame(request: Request):
    try:
        data = await request.json()
        frame = data["image"]
        print("frame---------->", frame)
        image = convert_to_im_array(frame)
        data_detcotor = detcotor.recognition(image)
        return {"name": data_detcotor}
    except:
        return {"name": "unknown"}
